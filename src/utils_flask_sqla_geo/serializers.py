...
def get_geoserializable_decorator(geoCol=None, idCol=None, **kwargs):
    ...
    def _geoserializable(cls):
        ...
        geom_exclude = [
            key for key, col in mapper.columns.items() if isinstance(col.type, Geometry)
        ]
        ...
        def populategeofn(self, geojson, recursif=True, col_geom_name="geom"):
            ...
            if not properties or not geometry or typeg != "Feature":
                raise UtilsSqlaError("Input must be a geofeature")
...
def geoserializable(*args, **kwargs):
    if not kwargs and len(args) == 1 and isinstance(args[0], type):  # e.g. @geoserializable
        ...
        return get_geoserializable_decorator(
            *args, **kwargs
        )  # e.g. @geoserializable(idCol='geom')
...
def shapeserializable(cls):
    ...
    def to_shape_fn(
        ...
        if fields:
            db_cols = [db_col for db_col in fields in cls.__mapper__.c if db_col.key in fields]
        else:
...
def txt_query_as_geojson(
    session, query, id_col, geom_col, geom_srid=4326, is_geojson=False, keep_id_col=False
):
    """
